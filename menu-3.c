...
int menu_pat_selectA(const struct menu_pat *tbl, int flag)
{
	...
	/***	タッチ入力	***/
	for(;;){
		tch_input(&x, &y, flag);		// flag==1 なら sw2 で break;
		for(i=0; tbl[i].w; i++){
			x1 = tbl[i].x;
			y1 = tbl[i].y;
			x2 = x1 + tbl[i].w -1;
...
int menu_keyA(const struct menu *tbl, int flag)
{
...
int menu_selectA0(const struct menu *tbl, int flag)
{
	int i, x, y, x1, y1, x2, y2;
...
{
	return menu_selectA0(tbl, 0);
}

int menu_select0_break(const struct menu *tbl)
{
	return menu_selectA0(tbl, 1);
}
/********************************************************************************/
/*		menu_select																*/
...
int menu_selectA(const struct menu *tbl, int flag)
{
	int i, x, y, x1, y1, x2, y2;
...
{
	return menu_selectA(tbl, 0);
}

int menu_select_break(const struct menu *tbl)
{
	return menu_selectA(tbl, 1);
}
/********************************************************************************/
/*		menu_select2															*/
...
int menu_select2(const struct menu *tbl)
{
	...
	/***	タッチパネル入力	***/
	for(;;){
		tch_input(&x, &y, 0);		// flag==1 なら sw2 で break;
		for(i=0; tbl[i].msg; i++){
//			lcd_clearA(tbl[i].x+4, tbl[i].y+3, tbl[i].x+(tbl[i].n+1)*12-4-1, tbl[i].y+26-3-1, btn_back_color);
			x1 = tbl[i].x+4;
			y1 = tbl[i].y+3;
...
//		sw_up();
		...
		else if(key & 1){					// sw1: execute
			return tbl[i].cmd;
		}
		else if(key & 2){					// sw2: abort(exit)
			return -1;
		}
	}
}
